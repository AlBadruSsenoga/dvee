...
{	
	...
	private function get_messages($message_type = FALSE, $target_user = 'public', $prefix_delimiter = FALSE, $suffix_delimiter = FALSE)
	{
		if (in_array($message_type, array('status', 'error')))
		{
			...
			{
				$status_prefix_delimiter = ($prefix_delimiter !== FALSE) ? $prefix_delimiter : $this->flexi->message_settings['delimiters']['error_prefix'];
				$status_suffix_delimiter = ($suffix_delimiter !== FALSE) ? $suffix_delimiter : $this->flexi->message_settings['delimiters']['error_suffix'];
			}
			else
			{
