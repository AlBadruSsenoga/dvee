	...
	<p>
		<div class="pull-left">
			<button type="submit" name="delete_selected" value="Delete Selected" class="btn btn-danger">
				<span class="glyphicon glyphicon-remove"></span> Selected
			</button>
			<a href="<?= site_url('admin/insert_user') ?>" class="btn btn-success">
				<span class="glyphicon glyphicon-plus"></span> User
			</a>
		</div>

		<div class="navbar-form pull-right" style="padding:0;margin:0;">
					...
					<tr id="<?= $user->id ?>" class="<?= ($this->ion_auth->is_admin($user->id)) ? 'danger' : '' ?>">
						...
						<td class="text-center">
							<div class="btn-group" style="width:100px">
								...
								<?= anchor('admin/update_user/'.$user->id, '<i class="glyphicon glyphicon-edit"></i>', 'class="btn btn-sm btn-info" title="Edit user information"') ?>
								<?php endif ?>
	...
	$(document).ready(function() {
		...
		$('.activate-user').click(function () {
			...
			$.ajax({
				...
				cache: true,
				beforeSend: function(){
					button.html('<span class="glyphicon glyphicon-refresh spinner"></span>')
				},
				success: function(response) {
	                ...
	                if(response.type) {
						if (response.type === 'success') {
							button.html('<i class="glyphicon glyphicon-ban-circle"></i>')
	                ...
	                if(response.type) {
						if (response.type === 'success') {
							...
							setTimeout(function(){
								...
								$('#ajax-alert-modal #message').html('Reloading Doctors <i class="glyphicon glyphicon-refresh spinner"></i>')
								$('#ajax-alert-modal').modal('show')
							...
							$('#ajax-alert-modal #message').html(response.message)
							$('#ajax-alert-modal').modal('show')
							button.html('<i class="glyphicon glyphicon-remove"></i>')
						}
					}
				},
				error: function() {
